...
class Board:
    ...
    def best_move(self, player):
        def alphabeta(node, depth, alpha, beta, play):
            if (depth == 0 or node.terminal() == True or node.moves(play) == [None]):
                return node.heuristic_value()
            '''if play == 1:
                value = -10000000
                for m in node.moves(play):
                    B = copy.deepcopy(node)
                    B.do_move(m, play)
                    value = max(value, alphabeta(B, depth - 1, alpha, beta, 1 - play))
                    if (value >= beta):
                        break
            ...
            for m in node.moves(play):
                B = copy.deepcopy(node)
                B.do_move(m, play)
                value = min(value, alphabeta(B, depth - 1, alpha, beta, 1 - play))
                if (value <= alpha):
                    break
                beta = min(beta, value)
                return value
        ...
        for m in self.moves(player):
            ...
            B.do_move(m, player)
            value = alphabeta(B, 3, -10000000, 10000000, 1 - player)
