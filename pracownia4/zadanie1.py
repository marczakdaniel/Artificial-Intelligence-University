...
VECTOR = [10, 801.724, 382.026, 78.922, 74.396, 10]
    ...
    def best_move(self, player):
        def alphabeta(node, depth, alpha, beta, player):
            if (depth == 0 or node.terminal() == True or node.moves(player) == [None]):
                return node.heuristic_value(VECTOR)
            if player == 1:
                value = -10000000
        ...
        for m in self.moves(player):
            ...
            B.do_move(m, player)
            value = alphabeta(B, DEPTH, -10000000, 10000000, 1 - player)
