...
def opt_dist(wiersz, D, j, czy_w, k):
    wynik1 = len(wiersz)

    poprawne = []
    if (czy_w):
        poprawne = zapisane_w[k]
    else:
        poprawne = zapisane_k[k]
    for p in poprawne:
        ile = 0
...
def losowa_zla_kolumna():
    global obrazek
    zle_w = []
    for i in range(kolumny):
        if zly_wiersz(kolumna_na_wiersz(i), D_kolumny[i]):
            zle_w.append(i)
    if len(zle_w) == 0:
        return -1
    return random.choice(zle_w)

def najlepsze_dopasowanie_w(i):
    w = obrazek[i]
    k = []
    min_dop = 0
    j_dop = -1
    for j in range(kolumny):
        ...
        wier1, wier2 = opt_dist(w, D_wiersze[i], j, 1, i)
        kol1, kol2 = opt_dist(k, D_kolumny[j], i, 0, j)
...
def najlepsze_dopasowanie_k(i):
    w = kolumna_na_wiersz(i)
    k = []
    min_dop = 0
    j_dop = -1
    for j in range(wiersze):
        ...
        kol1, kol2 = opt_dist(k, D_kolumny[i], j, 0, i)
        wier1, wier2 = opt_dist(w, D_wiersze[j], i, 1, j)
        przed_dop = wier1 + kol1
    ...
    for i in range(wiersze*kolumny*10):
        if random.randint(1, 2) == 1:
            ...
            if random.randint(1, 100) <= 25:
                ...
                k = najlepsze_dopasowanie_w(w)
                zamiana(w, k)
        else:
            k = losowa_zla_kolumna()
            if (k == -1):
                return True
            if random.randint(1, 100) <= 25:
                zamiana(random.randint(0, wiersze-1), random.randint(0,kolumny-1))
            else:
                w = najlepsze_dopasowanie_k(k)
                zamiana(w, k)
    print(1)
    return False
...
zapisane_w = []
for i in D_wiersze:
    zapisane_w.append(gen_row(kolumny, i))
zapisane_k = []
for i in D_kolumny:
    zapisane_k.append(gen_row(wiersze, i))
global obrazek

obrazek = tworz_losowy_obrazek()
wynik = rozwiazywanie()
while (wynik != True) or (sprawdzanie_kolumn() != True) or (sprawdzanie_wierszy() != True):
    obrazek = tworz_losowy_obrazek()
    wynik = rozwiazywanie()
